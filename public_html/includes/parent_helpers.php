...
function parent_layout_start(string $title, string $subtitle, string $activeSection, string $actionsHtml = '', ?string $breadcrumbExtra = null): void
{
    ...
    if ($hasExtra) {
        echo '<span class="admin-breadcrumb-page">' . parent_e($breadcrumb['page']) . '</span>';
        echo '<span class="admin-breadcrumb-sep" aria-hidden="true">&#8250;</span>';
        echo '<span class="admin-breadcrumb-page is-active">' . parent_e($breadcrumb['extra']) . '</span>';
    } else {
    ...
    echo '<a href="' . parent_e($logoutUrl) . '" class="admin-topbar-logout" title="Sign out" aria-label="Sign out">' . admin_action_icon('logout') . '</a>';
