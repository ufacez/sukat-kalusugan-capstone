...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	if ($action === 'update' && $parentId > 0) {
		$ok = admin_execute(
			'UPDATE parents SET name = ?, email = ?, phone = ?, address = ?, status = ? WHERE id = ?',
			'sssssi',
			[$name, $email, $phone, $address, $status, $parentId]
		);
	...
	if ($action === 'create') {
		...
		$ok = admin_execute(
			'INSERT INTO parents (name, email, password_hash, phone, address, status) VALUES (?, ?, ?, ?, ?, ?)',
			'ssssss',
			[$name, $email, $passwordHash, $phone, $address, $status]
		);
		...
		<table class="nutritionist-table" id="parents-table">
			...
			<tbody>
				<?php foreach ($parents as $parent): ?>
					<tr data-filter-text="<?php echo nutritionist_e(strtolower($parent['name'] . ' ' . $parent['email'] . ' ' . $parent['phone'] . ' ' . $parent['address'])); ?>">
						<td>
