...
if (
    ...
    admin_redirect('/admin/users.php', ['notice' => 'Enter a valid first name and surname (letters only). Middle name is optional.', 'type' => 'error']);
}
...
if (!admin_is_valid_ph_mobile($phone)) {
    admin_redirect('/admin/users.php', ['notice' => 'Enter a valid 11-digit PH mobile number starting with 09.', 'type' => 'error']);
}

$phone = preg_replace('/[^0-9]/', '', $phone);

if (!admin_is_strong_password($password)) {
    admin_redirect('/admin/users.php', ['notice' => 'Password must be at least 8 characters and include an uppercase letter, a lowercase letter, a number, and a special character.', 'type' => 'error']);
}

if ($password !== $passwordConfirm) {
    admin_redirect('/admin/users.php', ['notice' => 'Password and confirm password do not match.', 'type' => 'error']);
}
