...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_account'])) {
    ...
    if ($username === '' || preg_match('/^[A-Za-z0-9._]{3,30}$/', $username) !== 1) {
        admin_redirect('/admin/settings.php', ['notice' => 'Enter a valid username (3-30 characters: letters, numbers, dot, or underscore).', 'type' => 'error']);
    }
    ...
    if (admin_email_in_use($email, (int)$actor['id'], null)) {
        admin_redirect('/admin/settings.php', ['notice' => 'This email is already in use by another account. Use a different email address.', 'type' => 'error']);
    }
    if (admin_username_in_use($username, (int)$actor['id'])) {
        admin_redirect('/admin/settings.php', ['notice' => 'This username is already taken. Choose a different username.', 'type' => 'error']);
    }
